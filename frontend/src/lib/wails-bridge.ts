...
const raw = {
  ...
  // Reveal a saved file in the OS file manager. Called by name (not via the
  // generated ByID bindings) so it works without regenerating bindings; the Go
  // side resolves "main.App.RevealInFolder" and confines the path to the output
  // dir. No-op-safe if the backend predates this method (rejects harmlessly).
  revealInFolder: ((path: string) => Call.ByName("main.App.RevealInFolder", path)) as (
    path: string
  ) => Promise<void>,
